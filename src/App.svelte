<script>
    ...
    import ConnectionsPage from './routes/extras/connections.svelte';
    import RequisitionsPage from './routes/extras/requisitions.svelte';
    ...
    const routes = [
        ...
        {
            path: '/extras/connections',
            component: ConnectionsPage,
        },
        {
            path: '/extras/requisitions',
        ...
        {#if openFolder === folder || window.location.pathname.includes(folder)}
            <div class="relative pl-[1.7rem] space-y-1.5">
                ...
                {#each links as {link, color}}
                    <div class={`w-min pb-0.5 ${color} ${window.location.pathname.includes(link) && `border-b ${!color && borderColor}`}`}>
