<script>
    import { ArrowRightSLineArrows, CheckFillSystem, CloseFillSystem, CloseLargeLineSystem, OctagonFillDesign, StarFillSystem, StarLineSystem } from 'svelte-remix';
    import Triangle from '../../assets/svg/Triangle.svelte';
  
    const title = 'Ascent';
    const tagline = `Going up!`;
    const description = `
      <p>
...
<div class="w-full h-full py-8 pl-8 pr-24 space-y-8">
    ...
    {#each abilities as ability}
        <div class="space-y-3">
            <h2>{ability.title}</h2>
            <div>{@html ability.desc}</div>
            <div class="flex items-start space-x-2">
                <div class="pt-[0.2rem]">
                    <CheckFillSystem class="text-anomaly-blue size-[1rem]"/>
                </div>
                <div class="space-y-2">{@html ability.success}</div>
            </div>
            <div class="flex space-x-2">
                <div class="pt-[0.25rem]">
                    <StarLineSystem class="text-anomaly-blue size-[0.75rem]"/>
                </div>
                <div class="space-y-2">{@html ability.additional}</div>
            </div>
            <div class="flex space-x-2 pb-2">
                <div class="pt-[0.2rem]">
                    <CloseFillSystem class="text-agency-red size-[1rem]"/>
                </div>
                <div class="space-y-2">{@html ability.failure}</div>
            </div>
            <div class="space-y-1">
                <p><blue class="pr-1">Q: </blue>{ability.question}</p>
                <div class="space-y-0.5">
                    {#each ability.answers as {answer, code}}
